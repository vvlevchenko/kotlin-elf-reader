...
fun main(args: Array<String>) {
    ...
    val loader = ElfHeaderLoader.elfLoader(file)
    loader?.section(".shstrtab")?.let {
        val dump = (it as? ElfHeaderLoader.ElfStrTabSection)?.dumpTable()
        dump?.forEach {
            println(it)
        }
    }
    loader?.section(".debug.svm.imagebuild.arguments")?.let {
        val dump = loader.ElfStrTabSection(it as ElfHeaderLoader.ElfProgBitsSectionHeader).dumpTable()
        dump.forEach {
            println(it)
        }
    }
...
class ElfHeaderLoader(val bitness: BitnessHeaderOffsets, val buffer: MappedByteBuffer) {

    companion object {
        fun elfLoader(file: File): ElfHeaderLoader? {
    ...
    open inner class ElfSectionHeader(
        val offset: ULong
    ) {
        val nameIndex = readUInt(offset)
        val type = readUInt(offset + 4u)
        val sectionOffset = when(bitness) {
            BitnessHeaderOffsets.BITNESS_32 -> readUInt(offset + 16u).toULong()
            BitnessHeaderOffsets.BITNESS_64 -> readULong(offset + 24u)
        }
        val sectionSize = when(bitness) {
            BitnessHeaderOffsets.BITNESS_32 -> readUInt(offset + 20u).toULong()
            BitnessHeaderOffsets.BITNESS_64 -> readULong(offset + 32u)
        }
    }

    open inner class ElfProgBitsSectionHeader(offset: ULong) : ElfSectionHeader(offset)
    inner class ElfStrTabSection (offset: ULong) : ElfSectionHeader(offset) {
        /*constructor(header: ElfSectionHeader) : this(header.offset) {
            if (type != 0x3.toUInt())
                throw IllegalStateException("$type isn't string tab 0x3")
        }*/
        constructor(header: ElfProgBitsSectionHeader) : this(header.offset)
        fun readString(index : Int): String {
            return buffer.atOffset((sectionOffset + index.toUInt())) {
...
inline fun <reified T> ByteBuffer.atOffset(offset: ULong, action:ByteBuffer.() -> T): T {
